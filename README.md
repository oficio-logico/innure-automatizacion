...
`scripts/verify-innure-live.mjs` comprueba versión, páginas, recursos, rechazo de métodos y origen externo. Comprueba además que el receptor valida una opción nueva con un POST deliberadamente inválido, que termina antes de Turnstile y SMTP. No genera un lead. La recepción real de un mensaje se valida por separado con una única consulta técnica identificada, sin consentimiento publicitario.
